...
export const getBBoxFromPoint = point => {
  const {
    x,
    y
  } = point
...
export const getBBoxFromPoints = (points = []) => {
  const xs = []
  const ys = []
  points.forEach(p => {
    xs.push(p.x)
...
export const filterConnectPoints = points => {
...
export const simplifyPolyline = points => {
  points = filterConnectPoints(points)
  return points
}
export const getSimplePolyline = (sPoint, tPoint) => {
  return [
    sPoint,
...
export const getExpandedBBox = (bbox, offset) => {
...
export const isHorizontalPort = (port, bbox) => {
  const dx = Math.abs(port.x - bbox.centerX)
  const dy = Math.abs(port.y - bbox.centerY)
  return (dx / bbox.width) > (dy / bbox.height)
}
export const getExpandedBBoxPoint = (bbox, point) => {
...
export const getPointsFromBBox = bbox => {
  // anticlockwise
  const {
    minX,
    minY,
...
export const isPointOutsideBBox = (point, bbox) => {
  const {
    x,
    y
  } = point
...
export const getBBoxXCrossPoints = (bbox, x) => {
...
export const getBBoxYCrossPoints = (bbox, y) => {
...
export const heuristicCostEstimate = (p, ps, pt, source, target) => {
  return (distance(p, ps) + distance(p, pt)) + _costByPoints(p, [ps, pt, source, target])
}
export const reconstructPath = (pathPoints, pointById, cameFrom, currentId, iterator = 0) => {
...
export const isSegmentsIntersected = (p0, p1, p2, p3) => {
...
export const isSegmentCrossingBBox = (p1, p2, bbox) => {
...
export const getNeighborPoints = (points, point, bbox1, bbox2) => {
  const neighbors = []
  points.forEach(p => {
    if (p !== point) {
...
export const pathFinder = (points, start, goal, sBBox, tBBox, os, ot) => { // A-Star Algorithm
  const closedSet = []
  const openSet = [start]
  const cameFrom = {}
...
export const isBending = (p0, p1, p2) => {
  return !((p0.x === p1.x === p2.x) || (p0.y === p1.y === p2.y))
}
export const getBorderRadiusPoints = (p0, p1, p2, r) => {
...
export const getPathWithBorderRadiusByPolyline = (points, borderRadius) => {
  // TODO
  const pathSegments = []
...
export const getPolylinePoints = (start, end, sNode, tNode, offset) => {
